...
public class AiTestScoringStrategy implements ScoringStrategy {
    ...
    public UserAnswer doScore(List<String> choices, App app) throws Exception { // 计算分数
        Long appId = app.getId();//如果想计算分数 首先获取应用的id
        String jsonStr = JSONUtil.toJsonStr(choices);//将用户的选择转换成String对象
        String cacheKey = buildCacheKey(appId, jsonStr);// 用户生成一个缓存键，这个键值是由appid和用户的答案作为key的
        String answerJson = answerCacheMap.getIfPresent(cacheKey);// 去caffine本地缓存中查找是否由这个key的缓存结果
        // 如果有缓存，直接返回
